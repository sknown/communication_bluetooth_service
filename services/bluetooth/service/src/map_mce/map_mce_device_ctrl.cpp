    ...
    MasSdpParam &saveParam, const SdpService &servicePointer, uint16_t &l2capServerNum, bool &findL2capId)
{
    ...
    if ((!findMessageType) || (!findInstance)) {
        // param error
        ret = RET_BAD_PARAM;
        LOG_ERROR("%{public}s param error:inst=%{public}d,type:%{public}d",
            __PRETTY_FUNCTION__, findInstance, findMessageType);
    }
    return ret;
}
    ...
    MasSdpParam &saveParam, const SdpService &servicePointer, uint16_t &rfcommServerNum, bool findL2capId)
{
    ...
    if ((findRfcommId && (!findRfcommValue)) || !(findL2capId || findRfcommId)) {
        LOG_ERROR("%{public}s param error:l2id=%{public}d,rf=%{public}d,%{public}d",
            __PRETTY_FUNCTION__, findL2capId, findRfcommId, findRfcommValue);
        ret = RET_BAD_PARAM;
    }
    return ret;
}
...
int MceAnalyseSdpParam(MasSdpParam &saveParam, const SdpService &servicePointer)
{
    ...
    LOG_INFO("%{public}s execute finish:isL2cap=%{public}d,scn=0x%x,msgType=%{public}d,gapNumb=%{public}d,"
        "servicName=%{public}s", __PRETTY_FUNCTION__, saveParam.isGoepL2capPSM, saveParam.scn,
        saveParam.messageType, saveParam.gapNumber, saveParam.serviceName.c_str());
    return ret;
}
    ...
    const BtAddr *addr, const SdpService *serviceAry, uint16_t serviceNum, int finishId, int failedId)
{
    ...
    if ((serviceAry == nullptr) || (serviceNum == 0)) {
        ...
        LOG_ERROR("%{public}s service error,serviceAry is NUL or serviceNum =%{public}d",
            __PRETTY_FUNCTION__, int(serviceNum));
        return;
    }
    ...
    for (int serviceCount = 0; serviceCount < serviceNum; serviceCount++, servPtr++) {
        ...
        if ((servPtr->attribute == nullptr) || (servPtr->attributeNumber == 0) || (servPtr->descriptor == nullptr) ||
            ...
            mceService->PostMessage(msg);
            LOG_ERROR("%{public}s error:attrPtr is NULL or attrNum=%{public}d",
                __PRETTY_FUNCTION__, servPtr->attributeNumber);
            LOG_ERROR("%{public}s error:script is NULL or script Num=%{public}d",
                __PRETTY_FUNCTION__, servPtr->descriptorNumber);
            return;
        }
        // Start sdp analyse
...
void MapMceDeviceCtrl::SendRequest(utility::Message msg)
{
    ...
    if ((ctrlRequestPtr_ == nullptr) || (deviceInstanceStmMap_.size() == 0)) {
        LOG_ERROR("%{public}s ctrlRequestPtr_ is NULL or size=%{public}d",
            __PRETTY_FUNCTION__, int(deviceInstanceStmMap_.size()));
        return;
    }
