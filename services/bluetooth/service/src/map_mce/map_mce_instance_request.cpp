    ...
    MapMceInstanceClient &clientInst, const ObexHeader &resp, MapRequestResponseAction &retAction)
{
    ...
    LOG_INFO("%{public}s folderListing=%{public}s",
        __PRETTY_FUNCTION__, folderListingParam_.folderListingStringObject_.c_str());
#endif
    LOG_INFO("%{public}s end", __PRETTY_FUNCTION__);
    return MCE_RESPONSE_FINISH_NO_CALLBACK;
}
    ...
    MapMceInstanceClient &clientInst, const ObexHeader &resp, MapRequestResponseAction &retAction)
{
    ...
    if (rescode == uint8_t(ObexRspCode::SUCCESS)) {
        ...
        LOG_INFO("GetConversationListing stringObject=%{public}s",
            retAction.conversationList_.GetStringObject().c_str());
#endif
    }
    LOG_INFO("%{public}s end", __PRETTY_FUNCTION__);
    return MCE_RESPONSE_FINISH;
    ...
    MapMceInstanceClient &clientInst, const ObexHeader &resp, MapRequestResponseAction &retAction)
{
    ...
    if (rescode == uint8_t(ObexRspCode::SUCCESS)) {
        ...
        LOG_INFO("GetMessagesListing stringObject stream length=%{public}d,str obj "
            "length=%{public}d, msglist size=%{public}d",
