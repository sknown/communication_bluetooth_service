...
int MapMceInstanceClient::StartClientConnect()
{
    ...
    if (obexClientIns_ == nullptr) {
        ...
        LOG_INFO("%{public}s,new ObexMpClient, rawAddr=%{public}s,isL2cap=%{public}d,"
            "scn=0x%x,lpsm=0x%x,instId=%{public}d",
            __PRETTY_FUNCTION__,
...
int MapMceInstanceClient::RegisterServiceSecurity(GapServiceSecurityInfo &info)
{
    ...
    LOG_INFO("%{public}s gap execute,dir=%{public}d,servic=%{public}d,protocol=%{public}d,rfchannel=%{public}d,"
        "l2psm=0x%x,securityMode=0x%x",
        __PRETTY_FUNCTION__,
        info.direction,
        info.serviceId,
        info.protocolId,
