...
int PceConnectingState::ObexReconnect(const PbapPceObexMessage &obexMsg)
{
    ...
    if (header->GetFieldCode() != static_cast<uint8_t>(ObexRspCode::UNAUTHORIZED)) {
        if (client != nullptr) {
            ...
            PBAP_PCE_LOG_ERROR("%{public}s end, response is not UNAUTHORIZED Disconnect(not withObexReq)",
                __PRETTY_FUNCTION__);
        }
        return ret;
    }
    ...
    if (tvlHeader == nullptr) {
        PBAP_PCE_LOG_ERROR("%{public}s end, returned header from pse not contain challenge parameter",
            __PRETTY_FUNCTION__);
        return RET_NO_SUPPORT;
    }
