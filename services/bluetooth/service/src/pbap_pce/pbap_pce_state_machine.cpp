...
void PbapPceStateMachine::HandlePhoneBookSizeActionCompleted(const utility::Message &msg)
{
    ...
    if (IsResponeOK(respCode)) {
        PBAP_PCE_LOG_INFO("%{public}s respCode:0x%x, respTxt:%{public}s",
            __PRETTY_FUNCTION__, respCode, respTxt.c_str());
    } else {
        respTxt = "PhoneBookSize Error!";
        PBAP_PCE_LOG_ERROR("%{public}s respCode:0x%x, respTxt:%{public}s",
            __PRETTY_FUNCTION__, respCode, respTxt.c_str());
    }
